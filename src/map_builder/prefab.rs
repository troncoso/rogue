...
pub fn apply_prefab(mb: &mut MapBuilder, rng: &mut RandomNumberGenerator) {
    ...
    while placement.is_none() && attempts < 10 {
        ...
        dim.for_each(|pt| {
            ...
            if distance < 2000.0 && distance > 20.0 && mb.amulet_pos != pt {
                can_place = true;
            }
        });

        if can_place {
            placement = Some(Point::new(dim.x1, dim.y1));
            let points = dim.point_set();
        ...
        for ty in placement.y..placement.y + FORTRESS.2 {
            for tx in placement.x..placement.x + FORTRESS.1 {
                ...
                match c {
                    ...
                    _ => println!("Uknown tile character [{}]", c),
                }
                i += 1;
            }
        }
